...
class CKBisectingOperationManager {
    ...
    func addOperation() {
        ...
        os_log("Adding Bisecting Operation: %@ operations in progress", log: Log_CloudKit, type: .info, String(describing: runningOperationCount))
    ...
    func finishOperation(_ savedRecords: [CKRecord]?, _ deletedRecordIDs: [CKRecord.ID]?) {
        ...
        runningOperationCount -= 1
        os_log("Finished Bisecting Operation: %@ operations remaining", log: Log_CloudKit, type: .info, String(describing: runningOperationCount))
    }
...
class CKBisectingModifyRecordsOperation {
    ...
    var bisectingModifyRecordsCompletionBlock: ((_ totalRecordsSaved: [CKRecord]?, _ totalRecordsDeleted: [CKRecord.ID]?, _ error: Error?) -> Void)?
    
    func start(bisecting: Bool = false) {
        manager.addOperation()
        
        if bisecting {
            os_log("Beginning Bisecting Operation with %@ records", log: Log_CloudKit, type: .info, String(describing: recordBatchCount))
        }
        else {
            os_log("Beginning Modification Operation with %@ records", log: Log_CloudKit, type: .info, String(describing: recordBatchCount))
        }
    ...
    private func bisect(records: [CKRecord]?, recordIDs: [CKRecord.ID]?) {
        ...
        os_log("Batch too large. Bisecting %d records into two batches of %d and %d", log: Log_CloudKit, type: .info, savedRecords.count + deletedRecords.count, firstSaveBatch.count + firstDeleteBatch.count, secondSaveBatch.count + secondDeleteBatch.count)
        
        let firstOperation = CKBisectingModifyRecordsOperation(recordsToSave: firstSaveBatch, recordIDsToDelete: firstDeleteBatch, manager: manager, database: database)
        let secondOperation = CKBisectingModifyRecordsOperation(recordsToSave: secondSaveBatch, recordIDsToDelete: secondDeleteBatch, manager: manager, database: database)
